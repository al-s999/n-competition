...
import { Users, Gamepad2, Trophy, Search, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight } from "lucide-react";
                ...
                return (
                  <TableRow key={player.id}>
                    ...
                    <TableCell className="text-right">
                      <TimeAgo date={player.registeredAt} userTimezone={(player as any).timezone} className="text-xs text-muted-foreground" />
                    </TableCell>
                  </TableRow>
                );
              })
              ...
              <div key={player.id} className="rounded-lg border bg-card p-3 space-y-2">
                ...
                <div className="flex items-center justify-between text-xs text-muted-foreground pt-1 border-t border-border/50">
                  <button onClick={() => setSelectedPlayerForSessions(player)} className="flex items-center gap-1 hover:text-primary cursor-pointer transition-colors">
                    ...
                    <span className="text-primary font-medium">{player.gamesPlayed}</span>Play</button>
                  <button onClick={() => setSelectedPlayerForSessions(player)} className="flex items-center gap-1 hover:text-primary cursor-pointer transition-colors">
                    <Trophy className="h-3 w-3 text-yellow-500" />
                    <span className={`font-medium ${getScoreTextClass(player.avgScore)}`}>{player.avgScore.toFixed(1)}</span> avg
                  </button>
                  <TimeAgo date={player.registeredAt} userTimezone={(player as any).timezone} className="text-[10px]" />
                </div>
              </div>
            );
          })
                  ...
                  <div key={idx} className="flex flex-col gap-2 p-3 border rounded-lg bg-muted/20">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium text-sm truncate max-w-[160px] sm:max-w-[220px]" title={sess.quizTitle || sess.application || "Unknown Quiz"}>
