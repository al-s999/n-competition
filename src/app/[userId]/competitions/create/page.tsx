...
export default function AddCompetitionPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Mock save logic (replace Supabase)
    console.log("Mock saving competition:", newComp);
    if (matches.length > 0) {
      console.log("Mock saving matches:", matches);
    }

    // Simulate delay
    setTimeout(() => {
      showAlert("Success: Competition created (Mock)");
      setTimeout(() => {
        router.push(`/competitions/${newComp.id}`);
      }, 1000);
    }, 500);
  };
    ...
    <div className="w-full pb-12 relative">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <h1 className="text-2xl font-bold text-zinc-900 dark:text-white tracking-tight">Add</h1>
        </div>
